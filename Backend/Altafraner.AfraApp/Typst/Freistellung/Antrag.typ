...
#let status-labels = (
  ...
  Abgelehnt: "Abgelehnt",
  Genehmigt: "Genehmigt",
  Abgeschlossen: "Abgeschlossen (in Cevex eingetragen)",
)
...
// Elternbestätigung state
#if data.ElternbestaetigungErforderlich == true [
  === Elternbestätigung
  #if data.ElternbestaetigungVorhanden [
    Liegt vor.
  ] else [
...
// Full status history
  ...
  #table(
    ...
    table.header(
      [*Datum*], [*Person*], [*Status*], [*Kommentar*],
    ),
    ..for v in data.Verlauf {
      (
        format-datetime(v.Zeitpunkt),
        if v.Person != none { [#v.Person.Vorname #v.Person.Nachname] } else { [–] },
        status-labels.at(v.NeuerStatus, default: v.NeuerStatus),
        if v.Kommentar != none { v.Kommentar } else { [–] },
      )
    }
  )
