...
public class SchuljahrService
{
    ...
    public async Task<Domain.DTO.Schuljahr> GetSchuljahrAsync()
    {
        var schultage = await _dbContext
            ...
            .OrderBy(s => s.Datum)
            .Select(s => new DTO_Schultag(
                s.Datum,
                s.Wochentyp,
                s.Blocks.Select(b => new BlockSchema(
                    b.SchemaId,
                    _blockHelper.Get(b.SchemaId)!.Bezeichnung
                ))
            ))
            .ToListAsync();

        var next =
            schultage.FirstOrDefault(s => s.Datum >= DateOnly.FromDateTime(DateTime.Now))
    ...
    public async Task<Block?> GetCurrentBlockAsync()
    {
        ...
        var schultag = await _dbContext
            .Schultage.AsNoTracking()
            .Include(s => s.Blocks)
            .OrderBy(s => s.Datum)
    ...
    public async Task DeleteSchultagAsync(DateOnly datum)
    {
        ...
        if (schultag == null)
            throw new KeyNotFoundException("Schultag not found");
    ...
    public async Task<List<Models_Schultag>> AddRangeAsync(
        IEnumerable<SchultagCreation> schultageIn
    )
    {
        var blockKeys = _configuration.Value.Blocks.Select(e => e.Id).Distinct();
        var schultage = schultageIn
            .Select(s => new Models_Schultag
            {
                Datum = s.Datum,
                Wochentyp = s.Wochentyp,
                Blocks = s.Blocks.Select(b => new Block { SchemaId = b }).ToList(),
            })
            .ToList();

        if (schultage.SelectMany(s => s.Blocks).Any(b => !blockKeys.Contains(b.SchemaId)))
            throw new KeyNotFoundException(
                "Invalid block provided. Valid blocks are: " + string.Join(", ", blockKeys)
            );

        foreach (var schultag in schultage.ToList())
        {
            var conflict = await _dbContext
                .Schultage.Include(e => e.Blocks)
            ...
            if (
                conflict.Blocks.All(b1 => schultag.Blocks.Any(b2 => b1.SchemaId == b2.SchemaId))
                && schultag.Blocks.All(b1 => conflict.Blocks.Any(b2 => b1.SchemaId == b2.SchemaId))
            )
                continue;

            conflict.Blocks.AddRange(
                schultag.Blocks.Where(block =>
                    conflict.Blocks.All(b => b.SchemaId != block.SchemaId)
                )
            );
            conflict.Blocks.RemoveAll(block =>
                schultag.Blocks.All(b => b.SchemaId != block.SchemaId)
            );
        }
    ...
    public async Task<DateOnly?> GetLastDayWithBlocksAsync(DateOnly datum)
    {
        ...
        var day = await _dbContext
            .Blocks.Where(b => b.SchultagKey >= monday && b.SchultagKey < endOfWeek)
    ...
    private List<char> GetCurrentSchemas(TimeOnly now)
    {
        return _configuration
            .Value.Blocks.Where(metadata => metadata.Interval.Contains(now))
