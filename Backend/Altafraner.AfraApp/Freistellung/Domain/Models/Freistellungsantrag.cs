...
public class Freistellungsantrag
{
    ...
    /// <summary>
    ///     The current status of this request. Always changed together with appending an entry to
    ///     <see cref="Verlauf" /> — never set directly outside of
    ///     <c>FreistellungsService.TransitionAsync</c>.
    /// </summary>
    public FreistellungsStatus Status { get; set; } = FreistellungsStatus.Eingereicht;
    ...
    /// <summary>
    ///     The append-only history of status transitions for this request — the audit trail.
    ///     <see cref="Status" /> always equals the <c>NeuerStatus</c> of the most recent entry here.
    /// </summary>
    public List<FreistellungsVerlaufEintrag> Verlauf { get; set; } = [];
