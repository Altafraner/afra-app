...
public static class FreistellungsEndpoints
{
    ...
    {
        var sekretariat = await userAccessor.GetUserAsync();
        return await ExecuteAsync(() => service.EntscheidungElternbestaetigungAsync(sekretariat, antragId, dto));
    }
    ...
    {
        var sekretariat = await userAccessor.GetUserAsync();
        return await ExecuteAsync(() => service.CevexErledigtAsync(sekretariat, antragId));
    }
    ...
    private static async Task<IResult> SchulleiterBestaetigen(
        ...
        Guid antragId)
    {
        var schulleiter = await userAccessor.GetUserAsync();
        return await ExecuteAsync(() => service.SchulleiterBestaetigenAsync(schulleiter, antragId));
    }
    ...
    {
        var schulleiter = await userAccessor.GetUserAsync();
        return await ExecuteAsync(() => service.SchulleiterAblehnenAsync(schulleiter, antragId, dto));
    }
