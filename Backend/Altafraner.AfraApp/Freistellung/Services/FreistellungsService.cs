...
public class FreistellungsService
{
    ...
    /// <summary>
    ///     Records a teacher's or mentor's decision on a leave request. This does not by itself
    ///     approve or reject the overall request — only the Schulleiter can do that. Once every
    ///     teacher and mentor has decided (regardless of the individual outcome), the request
    ///     moves on to the Sekretariat.
    /// </summary>
    public async Task<FreistellungsantragDto> RecordEntscheidungAsync(Person lehrer, Guid antragId, EntscheidungDto dto)
    {
        ...
        if (AllDecided(antrag))
            antrag.Status = FreistellungsStatus.BeiSekretariat;
    ...
    {
        var antraege = await LoadAntraegeAsync(a => a.Status != FreistellungsStatus.Gestellt);
        return await BuildDtosAsync(antraege.OrderByDescending(a => a.Von).ToList());
    }
    ...
    /// </summary>
    public async Task<FreistellungsantragDto> EntscheidungElternbestaetigungAsync(Guid antragId,
        EntscheidungElternbestaetigungDto dto)
    {
        ...
        if (!dto.Erforderlich || dto.Vorhanden)
        {
            ...
            antrag.ElternbestaetigungHinweis = null;
            antrag.Status = FreistellungsStatus.BeimSchulleiter;
            await _dbContext.SaveChangesAsync();
        ...
        else
        {
            ...
            antrag.ElternbestaetigungHinweis = dto.Hinweis.Trim();
            antrag.Status = FreistellungsStatus.ElternbestaetigungAusstehend;
            await _dbContext.SaveChangesAsync();
            ...
            await NotifyAsync(antrag.Student, "Elternbestätigung erforderlich",
                $"""
                 ...
                 Hinweis vom Sekretariat: {antrag.ElternbestaetigungHinweis}
    ...
    public async Task<FreistellungsantragDto> ElternbestaetigungNachreichenAsync(Person student, Guid antragId)
    {
        ...
        if (antrag.Status != FreistellungsStatus.ElternbestaetigungAusstehend)
            throw new InvalidOperationException(
    ...
    /// <summary>
    ///     Marks a Schulleiter-approved leave request as entered into Cevex.
    /// </summary>
    public async Task<FreistellungsantragDto> CevexErledigtAsync(Guid antragId)
    {
        var antrag = await LoadAntragOrThrowAsync(antragId);

        if (antrag.Status != FreistellungsStatus.SchulleiterBestaetigt)
            throw new InvalidOperationException("Only Schulleiter-approved leave requests can be entered into Cevex.");

        antrag.InCevexEingetragen = true;
        await _dbContext.SaveChangesAsync();
    ...
    public async Task<FreistellungsantragDto> SchulleiterBestaetigenAsync(Guid antragId)
    {
        ...
        antrag.InCevexEingetragen = false;
        await _dbContext.SaveChangesAsync();
    ...
    public async Task<FreistellungsantragDto> SchulleiterAblehnenAsync(Guid antragId, AblehnungDto dto)
    {
        ...
        await NotifyAsync(antrag.Student, "Freistellungsantrag abgelehnt",
            $"""
             ...
             Kommentar: {antrag.SchulleiterKommentar}
             """);
    ...
    public async Task<int> GetOffeneAntraegeAnzahlAsync(Person person)
    {
        var anzahl = await _dbContext.Freistellungsantraege.CountAsync(a =>
            a.StudentId == person.Id && a.Status == FreistellungsStatus.ElternbestaetigungAusstehend);
        ...
        if (HatBerechtigung(person, GlobalPermission.Sekretariat))
            anzahl += await _dbContext.Freistellungsantraege.CountAsync(a =>
                ...
                || (a.Status == FreistellungsStatus.SchulleiterBestaetigt && !a.InCevexEingetragen));
    ...
    /// <summary>
    ///     Computes, for each given student, the number of Schulleiter-approved leave requests and
    ///     the number of missed lesson-hours they represent within the current Schuljahr.
    /// </summary>
        ...
        EntscheidungDto dto)
    {
        ...
        var hinweisZeile = dto.Status == EntscheidungsStatus.Genehmigt
            ? string.Empty
            : "\nDie endgültige Entscheidung über deinen Antrag trifft die Schulleitung.";
        await NotifyAsync(antrag.Student, $"Freistellungsantrag {entscheidungText}",
            $"""
             Dein Freistellungsantrag {AntragBezeichnung(antrag)} wurde von {entscheider.FirstName} {entscheider.LastName} {entscheidungText}.{kommentarZeile}{hinweisZeile}
