...
public class AttendanceService : IAttendanceService
{
    ...
    public async Task<OtiumAnwesenheitsStatus> GetAttendanceForEnrollmentAsync(Guid enrollmentId)
    {
        var enrollment = await _dbContext
            ...
            .Select(e => new { PersonId = e.BetroffenePerson.Id, BlockId = e.Termin.Block.Id })
            .FirstOrDefaultAsync();
        ...
        var attendance = await _dbContext
            .OtiaAnwesenheiten.Where(a =>
                a.StudentId == enrollment.PersonId && a.BlockId == enrollment.BlockId
            )
    ...
    /// <inheritdoc />
    public async Task<OtiumAnwesenheitsStatus> GetAttendanceForStudentInBlockAsync(
        Guid blockId,
        Guid personId
    )
    {
        var attendance = await _dbContext
            .OtiaAnwesenheiten.Where(a => a.StudentId == personId && a.BlockId == blockId)
            .Select(a => new { a.Status })
            .FirstOrDefaultAsync();
        return attendance?.Status ?? DefaultAttendanceStatus;
    }
    ...
    /// <inheritdoc />
    public async Task<Dictionary<Person, OtiumAnwesenheitsStatus>> GetAttendanceForTerminAsync(
        Guid terminId
    )
    {
        var blockIdWrapper = await _dbContext
        ...
        var isBlockRunning = schema
            .Interval.ToDateTimeInterval(DateOnly.FromDateTime(now))
            .Contains(now);
        ...
        if (isBlockRunning)
            personsQuery = personsQuery.Where(e =>
                e.Interval.Start <= time && e.Interval.End >= time
            );

        var persons = personsQuery.Select(e => e.BetroffenePerson).ToList();
        var personIds = persons.Select(p => p.Id).ToHashSet();

        var attendances = await _dbContext
            .OtiaAnwesenheiten.AsNoTracking()
        ...
        return persons.ToDictionary(
            p => p,
            p =>
                attendances.TryGetValue(p.Id, out var status)
                    ? status.Status
    ...
    {
        var block = await _dbContext
            .Blocks.AsNoTracking()
            .Where(b => b.Id == blockId)
            .Select(b => new
            {
                b.SchemaId,
                SindAnwesenheitenFehlernderErfasst = b.SindAnwesenheitenFehlernderKontrolliert,
            })
            .FirstOrDefaultAsync();
        ...
        var terminAttendance =
            new Dictionary<OtiumTermin, Dictionary<Person, OtiumAnwesenheitsStatus>>();
        ...
        var personIds = terminAttendance
            .Values.SelectMany(a => a.Keys)
            .Select(p => p.Id)
            .Distinct()
            .ToList();

        var missingPersons = await _dbContext
            .Personen.Where(p => p.Rolle == Rolle.Mittelstufe)
            .Where(p => !personIds.Contains(p.Id))
            .ToListAsync();

        var missingPersonIds = missingPersons.Select(p => p.Id).ToList();

        var missingPersonsAttendance = await _dbContext
            .OtiaAnwesenheiten.AsNoTracking()
        ...
        var missingPersonsAttendanceDict = missingPersons.ToDictionary(
            p => p,
            p =>
                missingPersonsAttendance.TryGetValue(p.Id, out var status)
                    ? status.Status
                    : DefaultAttendanceStatus
        );
        ...
        return (
            terminAttendance,
            missingPersonsAttendanceDict,
            block.SindAnwesenheitenFehlernderErfasst
        );
    }

    /// <inheritdoc />
    public async Task<Dictionary<Guid, OtiumAnwesenheitsStatus>> GetAttendanceForBlocksAsync(
        IEnumerable<Guid> blockIds,
        Guid personId
    )
    {
        var attendances = await _dbContext
            .OtiaAnwesenheiten.Where(a => a.StudentId == personId && blockIds.Contains(a.BlockId))
            .ToDictionaryAsync(a => a.BlockId, a => a.Status);

        return blockIds.ToDictionary(
            b => b,
            b => attendances.GetValueOrDefault(b, DefaultAttendanceStatus)
        );
    }

    /// <inheritdoc />
    public async Task SetAttendanceForEnrollmentAsync(
        Guid enrollmentId,
        OtiumAnwesenheitsStatus status
    )
    {
        var einschreibung = _dbContext
            .OtiaEinschreibungen.AsNoTracking()
            .Where(e => e.Id == enrollmentId)
    ...
    /// <inheritdoc />
    public async Task SetAttendanceForStudentInBlockAsync(
        Guid studentId,
        Guid blockId,
        OtiumAnwesenheitsStatus status
    )
    {
        var blockExists = await _dbContext.Blocks.AsNoTracking().AnyAsync(b => b.Id == blockId);
        if (!blockExists)
            throw new KeyNotFoundException($"Block ID {blockId} not found");

        var studentExists = await _dbContext
            .Personen.AsNoTracking()
    ...
    {
        var termin = await _dbContext.OtiaTermine.FirstOrDefaultAsync(t => t.Id == terminId);
        if (termin is null)
            throw new KeyNotFoundException($"Termin ID {terminId} not found");
    ...
    {
        var block = await _dbContext.Blocks.FirstOrDefaultAsync(b => b.Id == blockId);
        if (block is null)
            throw new KeyNotFoundException($"Block ID {blockId} not found");
    ...
    private bool IsInSupervisionTimeframe(Block block)
    {
        ...
        var supervisionInterval = new DateTimeInterval(
            dateTimeInterval.Start.Subtract(TimeSpan.FromHours(1)),
    ...
    {
        var attendance = await _dbContext.OtiaAnwesenheiten.FirstOrDefaultAsync(a =>
            a.StudentId == studentId && a.BlockId == blockId
        );
        ...
        {
            await _dbContext.OtiaAnwesenheiten.AddAsync(
                new OtiumAnwesenheit
                {
                    BlockId = blockId,
                    StudentId = studentId,
                    Status = status,
