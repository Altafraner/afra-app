...
public class RulesValidationService
{
    ...
    ///     violations.
    /// </summary>
    public async Task<List<string>> GetMessagesForEnrollmentsAsync(
        Person user,
    ...
    ///     violations.
    /// </summary>
    public async Task<List<string>> GetMessagesForWeekAsync(
        Person user,
    ...
    ///     violations.
    /// </summary>
    public async Task<List<string>> GetMessagesForDayAsync(
        Person user,
        Schultag schultag,
        List<OtiumEinschreibung> einschreibungen
    )
    {
        ...
        {
            var result = await rule.IsValidAsync(
                user,
                block,
        ...
        foreach (var priorityResult in priorityResults)
        {
            ...
            if (!priorityResult.Result.IsValid)
                messages.AddRange(
                    priorityResult.Result.Messages.Select(m => new MessageWithBlock(
                        m,
                        priorityResult.BlockId
                    ))
                );
        }
