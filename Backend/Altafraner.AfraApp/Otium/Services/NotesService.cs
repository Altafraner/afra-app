...
internal sealed class NotesService
{
    ...
    public NotesService(
        ...
        AttendanceRealtimeService attendanceRealtimeService
    )
    {
    ...
    public async Task<bool> TryAddNoteAsync(
        string content,
        Guid studentId,
        Guid blockId,
        Guid authorId
    ...
    {
        if (await HasNoteAsync(studentId, blockId, authorId))
            return false;

        await _dbContext.OtiaEinschreibungsNotizen.AddAsync(
            new OtiumAnwesenheitsNotiz
            {
                Content = content,
                AuthorId = authorId,
                StudentId = studentId,
    ...
    {
        var note = await _dbContext.OtiaEinschreibungsNotizen.FirstOrDefaultAsync(e =>
            e.StudentId == studentId && e.BlockId == blockId && e.AuthorId == authorId
        );
    ...
    {
        return await _dbContext.OtiaEinschreibungsNotizen.AnyAsync(e =>
            e.AuthorId == authorId && e.StudentId == studentId && e.BlockId == blockId
        );
    }
    ...
    {
        return await _dbContext.OtiaEinschreibungsNotizen.AnyAsync(e =>
            e.StudentId == studentId && e.BlockId == blockId
        );
    }
    ...
    public async Task<List<OtiumAnwesenheitsNotiz>> GetNotesAsync(Guid studentId, Guid blockId)
    {
        return await _dbContext
            .OtiaEinschreibungsNotizen.Include(e => e.Author)
