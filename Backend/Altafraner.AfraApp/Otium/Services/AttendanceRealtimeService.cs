...
internal sealed class AttendanceRealtimeService
{
    ...
    public AttendanceRealtimeService(
    ...
    internal async Task SendNoteUpdate(
        Guid studentId,
        Guid blockId,
        IEnumerable<OtiumAnwesenheitsNotiz> notes
    )
    {
        var termine = await _managementService.GetTermineInBlock(blockId);
        var update = new IAttendanceHubClient.NoteUpdate(
            studentId,
        ...
        foreach (var termin in termine)
            await _hubContext
                .Clients.Group(AttendanceHub.TerminGroupName(termin))
                .UpdateNote(update);
