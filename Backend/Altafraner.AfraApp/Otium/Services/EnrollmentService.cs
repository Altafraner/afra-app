...
internal class EnrollmentService
{
    ...
    public async Task<Models_OtiumTermin?> EnrollAsync(Guid terminId, Models_Person student)
    {
        var termin = await _dbContext
            .OtiaTermine.Include(termin => termin.Block)
    ...
    /// </exception>
    public async Task<MultiEnrollmentStatus> EnrollAsync(
        Guid terminId,
    ...
    )
    {
        ...
        var startingTermin = await _dbContext
            .OtiaTermine.Include(termin => termin.Block)
                .ThenInclude(block => block.Schultag)
            .Include(termin => termin.Otium)
                .ThenInclude(otium => otium.Kategorie)
        ...
        if (!mayEnroll.IsValid)
            throw new InvalidOperationException("Der Nutzer darf sich nicht einschreiben.");
        var einschreibung = new Models_OtiumEinschreibung
        {
        ...
        {
            var recurringTermin = startingTermin.Wiederholung?.Termine.FirstOrDefault(t =>
                t.Block.SchultagKey == date
            );
    ...
    public async Task<Models_OtiumTermin?> UnenrollAsync(
        Guid terminId,
        Models_Person student,
        bool force = false,
        bool save = true
    )
    {
        var enrollment = await _dbContext
            ...
            .FirstOrDefaultAsync(e =>
                e.BetroffenePerson.Id == student.Id && e.Termin.Id == terminId
            );
        ...
        if (sendNotification)
            await _notificationService.ScheduleNotificationAsync(
                enrollment.BetroffenePerson,
                "Abmeldung von Termin",
                $"""
                Du wurdest aus dem Termin {enrollment.Termin.Bezeichnung} am {enrollment.Termin.Block.Schultag.Datum:dd.MM.yyyy} im Block {_blockHelper.Get(
                    enrollment.Termin.Block.SchemaId
                )?.Bezeichnung
    ...
    public IEnumerable<Block> GetNotEnrolledBlocks(
        IEnumerable<Models_OtiumEinschreibung> enrollments,
        IEnumerable<Block> blocks
    )
    {
        var enrolledBlocks = enrollments.Select(e => e.Termin.Block.Id).Distinct();
    ...
    /// <returns></returns>
    public async Task<EinschreibungsPreview> GetEnrolmentPreview(
        Models_Person user,
        Models_OtiumTermin termin
    )
    {
        var terminEinschreibungen = await _dbContext
        ...
        var usersEnrollment = await _dbContext
            .OtiaEinschreibungen.Include(e => e.Termin)
        ...
        return new EinschreibungsPreview(
            countEnrolled,
    ...
    public async Task<int?> GetLoadPercent(Models_OtiumTermin termin)
    {
        ...
        var numEnrollments = await _dbContext
            .OtiaEinschreibungen.AsNoTracking()
    ...
    {
        var toTermin = await _dbContext
            .OtiaTermine.Include(t => t.Block)
            .FirstOrDefaultAsync(t => t.Id == toTerminId);
        if (toTermin == null)
            throw new KeyNotFoundException("Der Termin konnte nicht gefunden werden.");

        var currentEnrollment = await _dbContext
            .OtiaEinschreibungen.Include(e => e.Termin)
        ...
        await _dbContext.OtiaEinschreibungen.AddAsync(
            new Models_OtiumEinschreibung
            {
                BetroffenePerson =
                    await _dbContext.Personen.FindAsync(studentId)
        ...
        await _dbContext.SaveChangesAsync();
        return (GetOldTerminId(), toTermin.Block.Id);

        Guid GetOldTerminId()
        {
            var oldEnrollment = currentEnrollment.OrderBy(e => e.Interval.End).LastOrDefault();
    ...
    public async Task ForceMoveNow(Guid studentId, Guid fromTerminId, Guid toTerminId)
    {
        ...
        if (fromTerminId != Guid.Empty)
        {
            ...
            if (
                fromEinschreibung.Termin.Block.SchultagKey != today
                || !fromEinschreibung.Interval.Contains(nowTime)
            )
                throw new InvalidOperationException(
                    "Sie können keine Einschreibung jetzt beenden, wenn die Einschreibung nicht grade stattfindet!"
                );

            fromEinschreibung.Interval = new TimeOnlyInterval(
                fromEinschreibung.Interval.Start,
                nowTime
            );
        }
        ...
        _dbContext.OtiaEinschreibungen.Add(
            new Models_OtiumEinschreibung
            {
                BetroffenePerson =
                    await _dbContext.Personen.FindAsync(studentId)
    ...
    {
        var allBlocks = await _dbContext
            .Blocks.AsNoTracking()
        ...
        {
            var missingPersonsInBlock = await _dbContext
                .Personen.Where(p => p.Rolle != Rolle.Tutor)
                .Where(p =>
                    !_dbContext.OtiaEinschreibungen.Any(e =>
                        e.BetroffenePerson.Id == p.Id
    ...
    private async Task<RuleStatus> MayUnenroll(
        Models_Person user,
        Models_OtiumEinschreibung einschreibung
    )
    {
        ...
        var usersEnrollmentsInBlock = await GetPersonsEnrollmentsInBlock(
            user,
            einschreibung.Termin.Block
        );
        ...
        var (schultageInWeek, usersEnrollmentsInWeek) =
            await GetSchultageAndPersonsEnrollmentsInWeek(
                user,
        ...
        {
            var result = await rule.MayUnenrollAsync(
                user,
                schultageInWeek,
                usersEnrollmentsInWeek,
                einschreibung
            );
            if (!result.IsValid)
                return result;
    ...
    private async Task<RuleStatus> MayEnroll(Models_Person user, Models_OtiumTermin termin)
    {
        ...
        foreach (var rule in weekRules)
        {
            var result = await rule.MayEnrollAsync(
                user,
                schultageInWeek,
                usersEnrollmentsInWeek,
                termin
    ...
    private async Task<List<Models_OtiumEinschreibung>> GetPersonsEnrollmentsInBlock(
        Models_Person user,
        Block block
    )
    {
    ...
    private async Task<(
        ...
        List<Models_OtiumEinschreibung>
    )> GetSchultageAndPersonsEnrollmentsInWeek(Models_Person user, DateOnly dateInWeek)
    {
        var monday = dateInWeek.GetStartOfWeek();
        var endOfWeek = monday.AddDays(7);
        var usersEnrollmentsInWeek = await _dbContext
            .OtiaEinschreibungen.Include(e => e.Termin)
                .ThenInclude(t => t.Block)
            .Include(e => e.Termin)
                .ThenInclude(t => t.Otium)
            ...
            .Where(e =>
                e.Termin.Block.SchultagKey >= monday && e.Termin.Block.SchultagKey < endOfWeek
            )
            .ToListAsync();
        var schultageInWeek = await _dbContext
