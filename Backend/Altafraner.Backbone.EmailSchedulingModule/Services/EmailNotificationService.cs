...
internal class EmailNotificationService<TPerson> : INotificationService
    ...
    )
    {
        ...
        if (_dbContext is not DbContext contextActions)
            throw new InvalidOperationException(
                "The supplied database does not support this operation."
            );
        ...
        var job = JobBuilder
            .Create<BatchEmailsJob<TPerson>>()
            .WithIdentity(key)
            .UsingJobData("user_id", recipientId)
            .Build();
        var trigger = TriggerBuilder.Create().ForJob(key).StartAt(absDeadLine).Build();
