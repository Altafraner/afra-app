    ...
    <Popover ref="pop" :dismissable="true" :showCloseIcon="true" style="min-width: 15rem">
      <div class="p-3">
        <div class="flex items-center gap-3 mb-3">
          ...
          <Tag v-if="!person?.gruppe && person?.rolle" :value="person.rolle" rounded
               severity="info"/>
        ...
        <Button v-if="person?.email" :label="person.email" class="-ml-2" icon="pi pi-envelope"
                size="small"
                variant="text" @click.prevent="copy(person.email)"/>
        ...
        <template v-if="mentorsLoaded && mentors.length">
          ...
          <div class="mt-2 flex flex-col gap-2">
            ...
            <div v-for="mentor in mentors" :key="mentor.id">
              ...
              <Button v-if="mentor.email" :label="mentor.email" class="-ml-2" icon="pi pi-envelope"
                      size="small"
                      variant="text" @click.prevent="copy(mentor.email)"/>
            </div>
          </div>
        </template>
      </div>
