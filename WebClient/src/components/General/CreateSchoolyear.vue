...
function resolveStep1({values}) {
  ...
  if (!values.schoolyearStart)
    errors.schoolyearStart = [{message: 'Bitte geben Sie das Anreisedatum nach den Sommerferien an.'}]

  if (!values.schoolyearEnd)
    errors.schoolyearEnd = [{message: 'Bitte geben Sie das Abreisedatum in die Sommerferien an.'}]

  if (values.schoolyearStart && values.schoolyearEnd && values.schoolyearEnd <= values.schoolyearStart)
    errors.schoolyearEnd = [{message: 'Das Abreisedatum muss nach dem Anreisedatum liegen.'}]
...
async function submit() {
  ...
  while (current < end) {
    ...
    const angeboteAmWochentag = angeboteProWochentag.value.find(d => d.tag === ['Sonntag', 'Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag'][dayOfWeek]);
  ...
  <Stepper v-model:value="stepperStatus" linear>
    <StepItem value="0">
      ...
      <StepPanel v-slot="{activateCallback}">
        <p>
          Dieses Programm hilf ihnen, ihr Schuljahr vorauszuplanen.
        </p>
    ...
    <StepItem value="1">
      ...
      <StepPanel>
        ...
        <Form v-slot="$form" :resolver="resolveStep1" class="flex flex-col gap-4"
              @submit="form1Submit">
          <div class="w-full">
            <FloatLabel variant="on">
              <DatePicker id="schoolyearStart" v-model="schoolyearStart" date-format="dd.mm.yy"
                          fluid name="schoolyearStart" select-other-months show-icon/>
              <label for="schoolyearStart">Anreise nach den Sommerferien</label>
            </FloatLabel>
            <Message v-if="$form.schoolyearStart?.invalid" severity="error" size="small"
                     variant="simple">
          ...
          <div class="w-full">
            <FloatLabel variant="on">
              <DatePicker id="schoolyearEnd" v-model="schoolyearEnd" date-format="dd.mm.yy"
                          fluid name="schoolyearEnd" select-other-months show-icon/>
              <label for="schoolyearEnd">Abreise in die Sommerferien</label>
            </FloatLabel>
            <Message v-if="$form.schoolyearEnd?.invalid" severity="error" size="small"
                     variant="simple">
        ...
        <div class="flex flex-col gap-4">
          ...
          <div class="flex w-full gap-4 justify-stretch">
            <FloatLabel class="w-full" variant="on">
              <DatePicker v-model="newFerien" :disabled-dates="datesDisabled"
                          :max-date="schoolyearEnd" :min-date="schoolyearStart"
                          class="w-full" date-format="dd.mm.yy" select-other-months
                          selection-mode="range" show-button-bar show-icon/>
          ...
          <DataTable :value="ferienSorted">
            ...
            <Column class="afra-col-action text-right">
              <template #body="{data}">
                <Button icon="pi pi-trash" severity="danger" variant="text" @click="() => {
            ...
            <template #empty>
              <div class="flex justify-center">Sie können oben Ferientermine angeben.
              </div>
            </template>
          </DataTable>
          <div class="grid grid-cols-[1fr_4fr] gap-4">
            <Button label="Zurück" severity="secondary" @click="() => stepperStatus.value = '1'"/>
    ...
    <StepItem value="3">
      ...
      <StepPanel>
        <p>Wir haben automatisch versucht die H-/ N-Wochen vorherzusagen. Bitte kontrollieren Sie
          die Vorhersagen und passen Sie ggf. den Typ an.</p>
        <DataTable :value="weeks">
          ...
          <Column header="Typ">
            <template #body="{data}">
              <Select v-if="data.type === 'F'"
                      v-model="data.type" :options="[{label: 'Ferien', value: 'F'}]" disabled
                      fluid option-label="label" option-value="value"/>
              <Select v-else
                      v-model="data.type"
                      :options="[{label: 'H-Woche', value: 'H'}, {label: 'N-Woche', value: 'N'}]"
                      fluid option-label="label" option-value="value"/>
            </template>
          </Column>
        </DataTable>
        <Button class="mt-4" fluid label="Weiter"
    ...
    <StepItem value="4">
      ...
      <StepPanel>
        ...
        <Button class="mt-4" fluid label="Abschließen"
                @click="submit"/>
      </StepPanel>
    </StepItem>
  </Stepper>
