...
<template>
      ...
      <TabPanel header="Profunda">
        ...
        <DataTable :value="profunda" data-key="id">
          <Column header="Bezeichnung">
            <template #body="{ data }">
              <Button :label="data.bezeichnung" variant="text" as="RouterLink" :to="{
                name: 'Profundum-Edit',
          ...
          <Column class="text-right afra-col-action">
            <template #header>
              <Button v-tooltip.left="'Neues Profundum'" icon="pi pi-plus" aria-label="Neues Profundum"
                @click="createDialogOpen = true" />
            </template>

            <template #body="{ data }">
              <Button v-tooltip.left="'Löschen'" icon="pi pi-trash" severity="danger" variant="text"
                aria-label="Löschen" @click="deleteProfundum($event, data)" />
            </template>
          </Column>
  ...
  <Dialog v-model:visible="createDialogOpen" header="Neues Profundum anlegen" :modal="true" style="width: 40rem">
    <div class="flex flex-col gap-4">
      <div class="field">
        <label>Bezeichnung*</label>
        <InputText v-model="createModel.bezeichnung" maxlength="80" class="w-full" required />
      </div>

      <div class="field">
        <label>Kategorie*</label>
        <Dropdown v-model="createModel.kategorieId" :options="categories" optionLabel="bezeichnung" optionValue="id"
          placeholder="Kategorie auswählen" class="w-full" />
      </div>

      <div class="field">
        <label>Beschreibung</label>
        <Textarea v-model="createModel.beschreibung" rows="4" class="w-full" maxlength="1000" autoResize />
      </div>

      <div class="field">
        <label>Jahrgänge</label>
        <div class="flex gap-2 items-center">
          <InputText type="number" v-model.number="createModel.minKlasse" placeholder="min" class="w-20" />
          –
          <InputText type="number" v-model.number="createModel.maxKlasse" placeholder="max" class="w-20" />
        </div>
      </div>
    </div>
    ...
    <template #footer>
      ...
      <Button label="Anlegen" icon="pi pi-check" @click="createProfundum"
