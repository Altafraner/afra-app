...
watch(() => props.preSelected, (newValue, oldValue) => {
  if (newValue) {
...
<template>
  <div class="flex flex-col gap-3">
    <EinwahlSelector v-model="model[0]" :disabled="disabled[0]"
                     :options="optionsCleaned[0]"
                     :pre-selected="preSelected && preSelected[0] ? preSelected[0] : {}"
                     label="Erstwunsch"/>
    <EinwahlSelector v-model="model[1]" :disabled="disabled[1]"
                     :options="optionsCleaned[1]"
                     :pre-selected="preSelected && preSelected[1] ? preSelected[1] : {}"
                     label="Zweitwunsch"/>
    <EinwahlSelector v-model="model[2]" :disabled="disabled[2]"
